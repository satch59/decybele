...
class SocialShare implements ShortcodeInterface {
	...
	public function render($atts, $content = null) {
		...
		if ($params['enable_social_share']) {
			if ($params['enabled']) {
				$html .= mixtape_qodef_get_shortcode_module_template_part('templates/' . $params['type'], 'socialshare', '', $params);
			}
		}
	...
	private function getSocialNetworksParams($params) {
		...
		foreach ($this->socialNetworks as $net) {
			...
			if (mixtape_qodef_options()->getOptionValue('enable_'.$net.'_share') == 'yes') {
				...
				$html = mixtape_qodef_get_shortcode_module_template_part('templates/parts/network', 'socialshare', '', $params);
		...
		switch ($net) {
			case 'facebook':
				if(wp_is_mobile()){
					...
					$link = 'window.open(\'http://www.facebook.com/sharer.php?s=100&amp;p[title]=' . urlencode(mixtape_qodef_addslashes(get_the_title())) . '&amp;p[url]=' . urlencode(get_permalink()) . '&amp;p[images][0]=' . $image[0] . '&amp;p[summary]=' . urlencode(mixtape_qodef_addslashes(get_the_excerpt())) . '\', \'sharer\', \'toolbar=0,status=0,width=620,height=280\');';
				}
				break;
			case 'twitter':
				...
				if(wp_is_mobile()) {
					...
					$link = 'window.open(\'http://twitter.com/home?status=' . urlencode(mixtape_qodef_the_excerpt_max_charlength($count_char) . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');';
				}
				break;
			case 'google_plus':
