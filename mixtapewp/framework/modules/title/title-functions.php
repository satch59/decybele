...
if(!function_exists('mixtape_qodef_get_title')) {
    /**
     * Loads title area HTML
     */
    function mixtape_qodef_get_title() {
        $id = mixtape_qodef_get_page_id();

        extract(mixtape_qodef_title_area_height());
        extract(mixtape_qodef_title_area_background());
        ...
        if(($meta_temp = get_post_meta($id, "qodef_show_title_area_meta", true)) !== ""){
            ...
            $show_title_area = mixtape_qodef_options()->getOptionValue('show_title_area') == 'yes' ? true : false;
        }

        //check if breadcrumbs are enabled on page first, then in options
		$enable_breadcrumbs = mixtape_qodef_get_meta_field_intersect('title_breadcrumbs',$id) == 'yes' ? true : false;
        ...
        $parameters = apply_filters('mixtape_qodef_title_area_height_params', $parameters);

        mixtape_qodef_get_module_template_part('templates/title', 'title', '', $parameters);
    }
}

if(!function_exists('mixtape_qodef_get_title_text')) {
    /**
     * Function that returns current page title text. Defines mixtape_qodef_title_text filter
     ...
     */
    function mixtape_qodef_get_title_text() {
        ...
        if (is_tag()) {
            ...
            $title = single_term_title("", false).esc_html__(' Tag', 'mixtapewp');
        }
        ...
        elseif (is_author()) {
            ...
            $title = esc_html__('Author:', 'mixtapewp') . " " . get_the_author();
        }
		...
		elseif (is_search()) {
			...
			$title = esc_html__('Search Results for', 'mixtapewp').': '.get_search_query();
		}

        //is single post page
        elseif (is_singular('post')) {
            if (mixtape_qodef_options()->getOptionValue('blog_single_title_in_title_area') == "yes") {
        ...
        elseif (is_404()) {
            ...
            if(mixtape_qodef_options()->getOptionValue('404_title') != "") {
                //get it from options
                $title = mixtape_qodef_options()->getOptionValue('404_title');
            } else {
                //get default 404 page title
                $title = esc_html__('404 - Page not found', 'mixtapewp');
            }
        }

        //is WooCommerce installed and is shop or single product page?
        elseif(mixtape_qodef_is_woocommerce_installed() && (is_shop() || is_singular('product'))) {
        ...
        elseif(mixtape_qodef_is_woocommerce_installed() && (is_product_category() || is_product_tag())) {
            global $wp_query;
        ...
        elseif (is_archive()) {
            $title = esc_html__('Archive','mixtapewp');
        }
        ...
        $title = apply_filters('mixtape_qodef_title_text', $title);

        return $title;
    }
}

if(!function_exists('mixtape_qodef_title_text')) {
    /**
     * Function that echoes title text.
     *
     * @see mixtape_qodef_get_title_text()
     */
    function mixtape_qodef_title_text() {
        echo mixtape_qodef_get_title_text();
    }
}

if(!function_exists('mixtape_qodef_subtitle_text')) {
    /**
     * Function that echoes subtitle text.
     *
     */
    function mixtape_qodef_subtitle_text() {
        $id = mixtape_qodef_get_page_id();
        $subtitle 	= '';
...
if(!function_exists('mixtape_qodef_custom_breadcrumbs')) {
    /**
     ...
     * @see is_front_page()
     * @see is_category()
     * @see mixtape_qodef_is_product_category()
     * @see get_search_query()
     */
    function mixtape_qodef_custom_breadcrumbs() {
        global $post;

        $output = "";
        $homeLink = esc_url(home_url('/'));
        $pageid = mixtape_qodef_get_page_id();
        $bread_style = "";
        ...
        $delimiter = '<span class="qodef-delimiter" '.mixtape_qodef_get_inline_style($bread_style).'>/</span>'; // delimiter between crumbs
        $home = get_bloginfo('name'); // text for the 'Home' link
        $showCurrent = 1; // 1 - show current post/page title in breadcrumbs, 0 - don't show
        $before = '<span class="qodef-current" '.mixtape_qodef_get_inline_style($bread_style).'>'; // tag before the current crumb
        $after = '</span>'; // tag after the current crumb

        if (is_home() && !is_front_page()) {
            $output = '<div class="qodef-breadcrumbs"><div class="qodef-breadcrumbs-inner"><a '.mixtape_qodef_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a>' . $delimiter . ' <a '.mixtape_qodef_get_inline_style($bread_style).' href="' . $homeLink . '">'. get_the_title($pageid) .'</a></div></div>';
        ...
        elseif(is_front_page()) {
            if ($showOnHome == 1) $output = '<div class="qodef-breadcrumbs"><div class="qodef-breadcrumbs-inner"><a '.mixtape_qodef_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a></div></div>';
        }

        else {

            $output .= '<div class="qodef-breadcrumbs"><div class="qodef-breadcrumbs-inner"><a '.mixtape_qodef_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a>' . $delimiter;

            if ( is_category() || mixtape_qodef_is_product_category()) {
                ...
                $output .= $before . esc_html__('Search results for ', 'mixtapewp') . '"' . get_search_query() . '"' . $after;

            } elseif ( is_day() ) {
                $output .= '<a '.mixtape_qodef_get_inline_style($bread_style).' href="' . get_year_link(get_the_time('Y')) . '">' . get_the_time('Y') . '</a>' . $delimiter;
                $output .= '<a '.mixtape_qodef_get_inline_style($bread_style).' href="' . get_month_link(get_the_time('Y'),get_the_time('m')) . '">' . get_the_time('F') . '</a>' . $delimiter;
                ...
                $output .= '<a '.mixtape_qodef_get_inline_style($bread_style).' href="' . get_permalink($parent) . '">' . $parent->post_title . '</a>';
                ...
                while ($parent_id) {
                    ...
                    $breadcrumbs[] = '<a '.mixtape_qodef_get_inline_style($bread_style).' href="' . get_permalink($page->ID) . '">' . get_the_title($page->ID) . '</a>';
                ...
                $output .= $before . esc_html__('Posts tagged ','mixtapewp') .'"' . single_tag_title('', false) . '"' . $after;

            } elseif ( is_author() ) {
                global $authordata;
                $output .= $before . esc_html__('Articles posted by ','mixtapewp') . $authordata->display_name . $after;

            } elseif ( is_404() ) {
                $output .= $before . esc_html__('Error 404', 'mixtapewp') . $after;
            ...
            if ( get_query_var('paged') ) {
                ...
                $output .= $before . " (" . esc_html__('Page', 'mixtapewp') . ' ' . get_query_var('paged') . ")" . $after;
...
if(!function_exists('mixtape_qodef_get_title_area_height')) {

    /**
     * Function that returns title height
     **/
    function mixtape_qodef_get_title_area_height() {
        ...
        } elseif(mixtape_qodef_options()->getOptionValue('title_area_height') !== '') {
            $title_height = mixtape_qodef_options()->getOptionValue('title_area_height');
        }

        return apply_filters('mixtape_qodef_title_area_height', $title_height);
    }
}

if(!function_exists('mixtape_qodef_get_title_content_padding')) {
    /**
     * Function that returns title content pading
     **/

    function mixtape_qodef_get_title_content_padding() {
        return apply_filters('mixtape_qodef_title_content_padding', 0);
    }
}


if(!function_exists('mixtape_qodef_title_area_height')) {
    /**
     * Function that returns title height and padding to be applied in template
     **/

    function mixtape_qodef_title_area_height() {
        $id = mixtape_qodef_get_page_id();
        $title_height_and_padding = array();
        $title_height          = mixtape_qodef_get_title_area_height();
        $header_height_padding = mixtape_qodef_get_title_content_padding();
        ...
        if(($meta_temp = get_post_meta($id, "qodef_title_area_vertial_alignment_meta", true)) !== '') {
            ...
            $title_vertical_alignment = mixtape_qodef_options()->getOptionValue('title_area_vertial_alignment');;
        }
...
if(!function_exists('mixtape_qodef_title_area_background')) {
    /**
     * Function that returns title background style be applied in template
     **/

    function mixtape_qodef_title_area_background() {
        $id = mixtape_qodef_get_page_id();
        $show_title_img = true;
        ...
        if(($meta_temp = get_post_meta($id, "qodef_title_area_background_color_meta", true)) != ""){
            ...
            $background_color = mixtape_qodef_options()->getOptionValue('title_area_background_color');
        }
        ...
        if(($meta_temp = get_post_meta($id, "qodef_title_area_background_image_meta", true)) != ""){
            ...
            $background_image = mixtape_qodef_options()->getOptionValue('title_area_background_image');
        }

        //check for background image width
        $background_image_width = "";
        if($background_image !== ''){
            $background_image_width_dimensions_array = mixtape_qodef_get_image_dimensions($background_image);
