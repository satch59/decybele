...
class MixtapeQodeFramework {
    ...
    private function __construct() {
        $this->qodefOptions = MixtapeQodeOptions::get_instance();
        $this->qodefMetaBoxes = MixtapeQodeMetaBoxes::get_instance();
    }
	...
	public function setSkin(MixtapeQodeSkinAbstract $skinObject) {
		$this->skin = $skinObject;
	}
}

/**
 * Class MixtapeQodeSkinManager
 *
...
/**
 * Class MixtapeQodeSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class MixtapeQodeSkinAbstract {
    /**
     * @var string
     */
    protected $skinName;
    ...
    public function loadTemplatePart($template, $params = array()) {
        ...
        if($template !== '') {
            include(mixtape_qodef_find_template_path('framework/admin/skins/'.$this->skinName.'/templates/'.$template.'.php'));
        }
    }
    ...
    public function getMenuIcon($icon) {
		...
		return QODE_ASSETS_ROOT.'/img/admin-logo.png';
	}
    ...
    public function setShortcodeJSParams() { ?>
		<script>
			window.qodefSCIcon = '<?php echo mixtape_qodef_get_skin_uri().'/assets/img/admin-logo-icon.png'; ?>';
			window.qodefSCLabel = '<?php echo esc_html(ucfirst($this->skinName)); ?> Shortcodes';
		</script>
	<?php }
...
class MixtapeQodeOptions {
    ...
    public function getOptionValue($key) {
        ...
        if(array_key_exists($key, $mixtape_qodef_options)) {
            return $mixtape_qodef_options[$key];
...
/*
   Class: MixtapeQodeAdminPage
   A class that initializes Select Admin Page
*/
class MixtapeQodeAdminPage implements iMixtapeQodeLayoutNode {
    ...
    function __construct($slug="", $title="", $icon = "") {
        $this->layout = array();
        $this->factory = new MixtapeQodeFieldFactory();
        $this->slug = $slug;
        $this->title = $title;
        $this->icon = $icon;
    }
    ...
    public function renderChild(iMixtapeQodeRender $child) {
        $child->render($this->factory);
    }
}

/*
   Class: MixtapeQodeMetaBoxes
...
/*
   Class: MixtapeQodeMetaBox
   A class that initializes Qode Meta Box
*/
class MixtapeQodeMetaBox implements iMixtapeQodeLayoutNode {
	...
	public function setTitle($label) {
		...
		return $mixtape_qodef_Framework->getSkin()->getSkinLabel().' '.$label;
 	}
}

if ( ! function_exists( 'mixtape_qodef_init_framework_variable' ) ) {

	function mixtape_qodef_init_framework_variable() {

		global $mixtape_qodef_Framework;

		$mixtape_qodef_Framework = MixtapeQodeFramework::get_instance();
		$qodefSkinManager = new MixtapeQodeSkinManager();
		$mixtape_qodef_Framework->setSkin($qodefSkinManager->getSkin());
	}

	add_action('mixtape_qodef_before_options_map', 'mixtape_qodef_init_framework_variable' );
}
?>