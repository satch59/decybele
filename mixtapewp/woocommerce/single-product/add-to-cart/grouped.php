...
<form class="cart" method="post" enctype='multipart/form-data'>
            ...
            <tr id="product-<?php the_ID(); ?>" <?php post_class(); ?>>
                ...
                <td class="label">
                    <label for="product-<?php echo esc_attr($grouped_product->get_id()); ?>">
                        <?php print $product->is_visible() ? '<a href="' . esc_url( apply_filters( 'woocommerce_grouped_product_list_link', get_permalink(), $grouped_product->get_id() ) ) . '">' . get_the_title() . '</a>' : get_the_title(); ?>
                    </label>
                </td>
    ...
    <?php if ( $quantites_required ) : ?>
        ...
        do_action('mixtape_qodef_woocommerce_add_to_cart_button');
        ?>
